const GEMINI_API_URL = `...?key=AIzaSyAzV4HVZnxhmfEeRhcaa-rGUHtr7M7wk_w`;
...
export async function getSnackRecommendation(caloriesExpended) {
    ...
    try {
        ...
        if (!response.ok) {
            console.warn("Gemini APIが失敗したため、デモ用データを使用します。ステータス:", response.status);
            // 本番環境のエラーであれば、ランダムにデモデータを返す
        ...
        console.warn("通信エラーが発生したため、デモ用データを使用します:", error);
        return MOCK_SNACKS[Math.floor(Math.random() * MOCK_SNACKS.length)];
    }
}